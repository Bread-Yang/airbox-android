...
public class WeatherContentProvider extends ContentProvider {
	public static final Uri CONTENT_URI = Uri
			.parse("content://com.haier.airbox.provider.WeatherContentProvider");

	public static class Columns {
		public static final String ID = "_id"; // 如果正在使用Android数据库，必须为其定义一个叫_id的列，它用来表示每条记录的唯一性。
	...
	public synchronized Cursor query(Uri uri, String[] projection,
		...
		final MatrixCursor c = new MatrixCursor(new String[] { Columns.ID,
				Columns.DAY, Columns.TEMPERATURE });
		for (int i = 0; i < sData.size(); ++i) {
			final WeatherDataPoint data = sData.get(i);
			c.addRow(new Object[] { new Integer(i), data.day,
					new Integer(data.degrees) });
		}
		return c;
	}
